...
# Simplified version to avoid quote escaping issues
...
if ($envVars.ContainsKey("VITE_TWITTER_REDIRECT_URI")) {
    ...
    if ($redirectUri -match "localhost" -or $redirectUri -eq "") {
        ...
        if ($ProductionUrl -ne "") {
            ...
            Write-Host "✅ Updated VITE_TWITTER_REDIRECT_URI" -ForegroundColor Green
        }
    }
}
...
$varsToFix = $envVars.GetEnumerator() | Where-Object { 
    $_.Value.NeedsFix -or 
    $_.Value.Cleaned -match "localhost" -or 
    ($_.Key -eq "VITE_TWITTER_CLIENT_ID" -and $_.Value.Cleaned.Length -lt 10) 
}
...
foreach ($var in $varsToFix) {
    ...
    $origLen = [Math]::Min(60, $var.Value.Original.Length)
    $cleanLen = [Math]::Min(60, $var.Value.Cleaned.Length)
    Write-Host "    Before: $($var.Value.Original.Substring(0, $origLen))..." -ForegroundColor Red
    Write-Host "    After:  $($var.Value.Cleaned.Substring(0, $cleanLen))..." -ForegroundColor Green
}
...
foreach ($var in $varsToFix) {
    ...
    foreach ($env in $environments) {
        ...
        # Add new variable
        $newValue | npx vercel env add $varName $env 2>&1 | Out-Null
